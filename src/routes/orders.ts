...
import { invalidateProduct } from "../services/productCache.js";
    ...
    try {
      const newOrder = await prisma.$transaction(async (tx) => {
        ...
        const totalAmount = validatedOrder.items.reduce(
          (sum, i) => sum + byId.get(i.productId)!.price * i.quantity,
          0,
        );
        ...
        for (const item of validatedOrder.items) {
          ...
          if (result.count === 0)
            throw new OrderInputError(
              "Stock changed while ordering , try again",
            );
      ...
      return res
        .status(500)
        .json({ message: "Internal server error while creating the order" });
    }
  },
);
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (!existing)
        return res
          .status(404)
          .json({ message: "Order to be updated not found" });
      ...
      const updatedOrder = await prisma.$transaction(async (tx) => {
        ...
        if (items) {
          ...
          for (const oldItem of current!.items) {
            ...
            await invalidateProduct(oldItem.productId);
          }
          ...
          for (const item of items) {
            ...
            if (p.stock < item.quantity)
              throw new OrderInputError(`Only ${p.stock} left of "${p.name}"`);
          }
          ...
          for (const item of items) {
            ...
            if (r.count === 0)
              throw new OrderInputError(
                "Stock changed while updating — try again",
              );
            await invalidateProduct(item.productId); //invalidate product in cache since its stock changed
          }
      ...
      return res
        .status(200)
        .json({ order: updatedOrder, message: "Order updated successfully" });
    } catch (e) {
      if (e instanceof OrderInputError)
        return res.status(400).json({ message: e.message });
    ...
    try {
      ...
      const deletedOrder = await prisma.$transaction(async (tx) => {
        ...
        if (order.status !== "pending")
          throw new OrderInputError("Only pending orders can be canceled");
